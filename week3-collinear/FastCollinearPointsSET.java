/**
 * в решении запрещено использовать SET (
 */
...
public class FastCollinearPointsSET {
    // finds all line segments containing 4 or more points

    private final ArrayList<LineSegment> segments = new ArrayList<>();
    private class PointSlope implements Comparable{
        private final Point endPoint;
        private final double slope;
        PointSlope(Point point, double slope) {
            this.endPoint = point;
            this.slope = slope;
        }
        ...
        public int compareTo(Object obj) {
            ...
            if (this.endPoint == that.endPoint && this.slope == that.slope)
                return 0;
            else if (this.endPoint.compareTo(that.endPoint) == 0)
    ...
    public FastCollinearPointsSET(Point[] points) {
        checkNull(points);
        ...
        // сет конечных для отброса дублей.
        SET setEndPoints = new SET();
        ...
        for (int n = 0; n < nps.length - 1; n++) {
            ...
            for (Point point: endPoints) {
                if (!setEndPoints.contains(new PointSlope(point, p.slopeTo(point)))) {
                // if (checkEndPoint(point, p.slopeTo(point), fromPoint, toPoint)) {
                    setEndPoints.add(new PointSlope(point, p.slopeTo(point)));
                    fromPoint.add(p);
                    toPoint.add(point);
    ...
    public static void main(String[] args) {
        ...
        FastCollinearPointsSET collinear = new FastCollinearPointsSET(points);
